 ...
 *@mm: memory region
 *@rgid: region ID act as symbol index of variableư
 *
 */
...
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
{
  ...
  /* Đọc giá trị từ bộ nhớ RAM */
  struct sc_regs regs;
  regs.a1 = SYSMEM_IO_READ;
  regs.a2 = phyaddr; // Địa chỉ vật lý
  regs.a3 = 0;       
  regs.orig_ax = 17; // Syscall sys_memmap
  ...
  /* Cập nhật giá trị đọc được */
  *data = (BYTE)regs.a4; // Giả định giá trị đọc được trả về qua regs.a4
...
int find_victim_page(struct mm_struct *mm, int *retpgn)
{
  ...
  *retpgn=pg->pgn;
  if (prev == NULL)
    mm->fifo_pgn = NULL; // Danh sách chỉ có 1 node
  else
    prev->next = NULL;
  free(pg);
